// github.com/bellbind/stepbystep-timsort
// TimSort: Put It Together
// - combine codes of both split and merge improvement
aij.timSort = function (array) {

    var MIN_GALLOP = 7;

    // === [SPLIT IMPROVEMENTS] === //
    // timsort: main loop
    var timSort = function (array, first, last) {
    ...
    var nextRun = function (state) {
        ...
        if (last - state.remain < state.minrun) {
            ...
            binarySort(state.array, state.remain, last, sortStart);
        }
        cutRun(state, last);
        return true;
    };
    ...
    var mergeIntoLeft = function (array, first, connect, last, state) {
        ...
        m.cur = binSearch(array, first, connect, m.right[m.rcur]);
    ...
    var mergeLeftGallopingMode = function (array, state, m) {
        ...
        if (lval <= rval) {
            ...
            end = gallopFirstSearch(m.left, m.lcur + 1, m.llast, rval);
            ...
            end = gallopFirstSearch(m.right, m.rcur + 1, m.rlast, lval);
    ...
    var mergeIntoRight = function (array, first, connect, last, state) {
        ...
        // find merge start point which is insert point for first of larger side
        m.cur = binSearch(array, connect, last, m.left[m.lcur - 1]);
    ...
    var mergeRightGallopingMode = function (array, state, m) {
        ...
        if (rval <= lval) {
            ...
            begin = gallopLastSearch(m.left, m.lfirst, m.lcur - 1, rval);
            ...
            begin = gallopLastSearch(m.right, m.rfirst, m.rcur - 1, lval);
    ...
    var gallopFirstSearch = function (array, first, last, value) {
        var pre = 0;
        var offset = 1;
        while (first + offset < last) {
            if (value < array[first + offset]) break;
            pre = offset;
            offset = (offset << 1) + 1;
        }
        var searchFirst = first + pre;
        var searchLast = (first + offset < last) ? first + offset : last;
        return binSearch(array, searchFirst, searchLast, value);
    };

    // binsearch for gallop mode from last element side
    // search to one of regions(from last) [-1,-0),[-3,-1),[-7,-3),[-15,-7),...
    var gallopLastSearch = function (array, first, last, value) {
        var pre = 0;
        var offset = 1;
        while (first < last - offset) {
            if (value >= array[last - offset]) break;
            pre = offset;
            offset = (offset << 1) + 1;
        }
    ...
    // binary search
    var binSearch = function (array, first, last, value) {
        while (first < last) {
    ...
    var binarySort = function (array, first, last, sortStart) {
        sortStart = sortStart || first + 1;
        for (var i = sortStart; i < last; i += 1) {
            var point = binSearch(array, first, i, array[i]);
